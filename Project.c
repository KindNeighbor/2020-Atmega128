...
ISR(TIMER0_OVF_vect) { // 오버 플로우 타이머를 이용해서 숫자가 카운팅 되도록 한다.  
	if(MODE==MODE1) {
		switch(state) { // 외부 인터럽트에 따라서 동작을 나눔
			...
			case START :
			count_int++;
			if(count_int == 244) { // 244번에 한번씩 SHOW_NUMBER34를 증가시킨다. 밑의 코드에서 오버 플로우 타이머 레지스터를 설정해서 
					       // 약 244Hz를 만들 것이므로,  244번에 한번 올라감 = 1초를 만든다.
				SHOW_NUMBER34++;
	...
	while(1) {
		switch(MODE) { // 모드에 따른 작동을 구분하기 위해 스위치 함수 사용
			...
			case MODE2 : 
				...
				SHOW_NUMBER_1 = SHOW_NUMBER1*1000+SHOW_NUMBER2*100+SHOW_NUMBER3*10+SHOW_NUMBER4; // 표시되는 숫자를 각 자리 숫자에 
														// 10~1000을 각각 곱해서 표현
