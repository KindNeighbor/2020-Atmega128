...
int main(void) {
	...
	TCNT0 = 0x00; // 0부터 시작해서 255까지 카운트 되고, 256이 되면 0이 되면서 오버플로우 인터럽트가 발생한다. 256번을 카운트 하므로, 오버 플로우가 발생하는 간격은 62500Hz/256 = 244.14Hz로 약 244Hz가 된다. 위에서 count_int를 가지고 시간의 속도를 정할 때 244란 숫자가 나오는 이유이다. 
...
void ShowDigit(int i, int digit) {
	...
	if(digit==1)
		PORTA = Segment_Data[i]|0x80; // 위에서 언급했듯이 분과 초 사이에 점을 표시 하기 위해서 설정. 비트 연산자를 통해서 digit=1일 때, 즉 세그먼트의 왼쪽에서 두번째 자리의 숫자 옆의 점을 8번째 비트를 1로 할당 시키고 | 연산자, 즉 or연산을 통해서 점을 항상 켜 놓도록 한다.
	else
